...
exports.getOrCreateDocument = function (documentId, callback) {
	getDoc(documentId, function(err, data) {
		console.log( "GETORCREATEDOCUMENT(): " + documentId);
		if (err === null && data === null) {
			...
			doc.save(function (e, saved) {
				...
				else { console.log("Saved: " + saved._id); }
				callback(e, saved);
			});
		}
		else {
			if (data !== null) { console.log( "RETRIEVED: " + documentId ); }
			callback(err, data);
		}
	});
}
...
var saveDoc = function (newText, dbDocument, callback) {
	...
	if (newText !== dbDocument.text) {
		...
		console.log("Saving document change \"" + dbDocument._id + "\"...");
		...
		dbDocument.save(function (err, saved) {
			...
			console.log("Saved document: \"" + dbDocument._id + "\"");
			callback(err, saved);
		});
	}
	else {
...
var emitChange = function (document, emailAddress, socket, callback) {
	...
	socket.broadcast.to(document._id).emit('edit', change);
	console.log("CHANGE DOCUMENT: " + document._id + ", " + emailAddress);
};
